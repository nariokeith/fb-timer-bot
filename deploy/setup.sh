#!/usr/bin/env bash
# First-time provisioning for a fresh Ubuntu VM (Google Cloud, Oracle, any VPS).
#
...
# Whoever ran `sudo bash setup.sh`. Oracle's Ubuntu images create
# `ubuntu` and Google Cloud's do not -- theirs is named after the Google
# account -- so this is derived rather than assumed. Override by exporting
# APP_USER if the bots should run as someone else entirely.
...
log "Installing the systemd unit as ${APP_USER}"
if ! id -u "${APP_USER}" >/dev/null 2>&1; then
  echo "No such user: ${APP_USER}. Re-run with sudo, or export APP_USER." >&2
  exit 1
fi
sed "s/__APP_USER__/${APP_USER}/" \
  "${APP_DIR}/deploy/fb-timer-bot.service" > /etc/systemd/system/fb-timer-bot.service
chmod 644 /etc/systemd/system/fb-timer-bot.service
